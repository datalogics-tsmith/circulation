...
from core.theta import (
    ThetaAPI as BaseThetaAPI,
    MockThetaAPI as BaseMockThetaAPI,
    ThetaParser,
    BibliographicParser,
    ThetaBibliographicCoverageProvider
)
...
class ThetaAPI(BaseThetaAPI, Authenticator, BaseCirculationAPI):
    ...
    def update_licensepools_for_identifiers(self, identifiers):
        ...
        # We asked Theta about n books. It sent us n-k responses. Those
...
class ThetaCirculationMonitor(Monitor):

    """Maintain LicensePools for Theta titles.
    """

    VERY_LONG_AGO = datetime(1970, 1, 1)
    FIVE_MINUTES = timedelta(minutes=5)

    def __init__(self, _db, name="Theta Circulation Monitor",
                 interval_seconds=60, batch_size=50):
        super(ThetaCirculationMonitor, self).__init__(
        ...
        self.bibliographic_coverage_provider = (
            ThetaBibliographicCoverageProvider(self._db)
        )

    def run(self):
        self.api = ThetaAPI(self._db)
        super(ThetaCirculationMonitor, self).run()
    ...
    def process_book(self, bibliographic, availability):
        ...
        if new_license_pool or new_edition:
            ...
            # the ThetaBibliographicCoverageProvider. Register that the
...
class MockThetaAPI(BaseMockThetaAPI, ThetaAPI):
    pass

class ThetaCollectionReaper(IdentifierSweepMonitor):
    """Check for books that are in the local collection but have left our
    Theta collection.
    """

    def __init__(self, _db, interval_seconds=3600*12):
        super(ThetaCollectionReaper, self).__init__(
            _db, "Theta Collection Reaper", interval_seconds)

    def run(self):
        self.api = ThetaAPI(self._db)
        super(ThetaCollectionReaper, self).run()

    def identifier_query(self):
        return self._db.query(Identifier).join(
            Identifier.licensed_through).filter(
                Identifier.type==Identifier.THETA_ID).options(
...
class ResponseParser(ThetaParser):

    id_type = Identifier.THETA_ID

    SERVICE_NAME = "Theta"

    # Map Theta error codes to our circulation exceptions.
    code_to_exception = {
    ...
    def raise_exception_on_error(self, e, ns, custom_error_classes={}):
        ...
        try:
            ...
            raise RemoteInitiatedServerError(
                "Invalid response code from Theta: %s" % code,
                self.SERVICE_NAME
            )
